...
if st.sidebar.button("Fetch Stock Data"):
    ...
    if data is not None:
        ...
        rsi = 100 - (100 / (1 + rs))
        latest_rsi = rsi.iloc[-1] if not rsi.isna().iloc[-1] else 50
        ...
        # Trend detection
        is_bullish = latest_price > ma_50 > ma_100 > ma_200
        is_bearish = latest_price < ma_50 < ma_100 < ma_200

        # Final recommendation
        if is_bullish and 40 < latest_rsi < 70 and avg_esg_score >= 75:
            recommendation = f"BUY ({esg_strength})"
        elif is_bearish and (latest_rsi < 30 or latest_rsi > 70) and avg_esg_score < 60:
        ...
        st.markdown(f"<h6>RSI: {latest_rsi:.2f}</h6>", unsafe_allow_html=True)
